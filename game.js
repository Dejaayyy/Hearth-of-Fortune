...
function promptForClueType() {
    ...
    if (clueTries >= 3) {
        document.getElementById("noclueleft").classList.toggle("active");
        return;
    }
...
function hintclue() {
    ...
    document.getElementById("hc-content").style.height = '260px';
}
    ...
    if (remainingLetters.length > 0) {
        ...
        if (choice === 'C') {
            ...
            if (consonants.length > 0) {
                letterToReveal = consonants[0].letter; // Only reveal the first consonant found
            } else {
            ...
            if (vowels.length > 0) {
                letterToReveal = vowels[0].letter; // Only reveal the first vowel found
            } else {
        ...
        for (let i = 0; i < currentAnswer.length; i++) {
            if (currentAnswer[i] === letterToReveal) {
                guessedWord[i] = letterToReveal;
            }
        }

        // Disable and turn gray the button corresponding to the revealed letter
        const letterButtons = document.querySelectorAll('.letter');
        letterButtons.forEach(button => {
            if (button.innerText === letterToReveal) {
                button.style.backgroundColor = 'gray';
        ...
        showCustomAlert(`Clue: The missing letter is '${letterToReveal}'.`);
    } else {
